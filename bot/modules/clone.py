...
def _clone(message, bot, multi=0):
    ...
    if is_gdrive_link(link):
        ...
        if STOP_DUPLICATE:
            LOGGER.info('Bro Checking File/Folder if already in Drive ')
            smsg, button = gd.drive_list(name, True, True)
            if smsg:
                msg3 = "Bro File/Folder is already available in Drive.\n results - "
                return sendMarkup(msg3, bot, message, button)
        if CLONE_LIMIT is not None:
            LOGGER.info('Wait Checking File/Folder Size ')
        ...
        sendMessage('Bro Send GOOGLE DRIVE - GOTOT - APPDRIVE - link with cmd by replying to the link by cmd', bot, message)
